...
Projects move through a fixed sequence:
...
- it no longer gives the coding agent a broad task, and instead requires each mission block to define allowed files, forbidden files, acceptance checks, and rollback notes
...
2. Every implementation task must have a `Mission Block`.
3. The Builder must not go beyond `allowed_files`.
4. Review must use structured output.
5. `SESSION_STATE.md` must be updated before every session ends.
6. If the user provides only repository plus scene, the agent must bootstrap from repository rules instead of asking for internal document paths again.
...
- 不再给编码代理宽泛任务，而是要求每个 mission block 都定义允许文件、禁止文件、验收检查和回滚说明
...
2. 每个实现任务必须有 `Mission Block`
3. Builder 不得越出 `allowed_files`
4. 审查必须使用结构化输出
5. 每次会话结束前必须更新 `SESSION_STATE.md`
6. 如果用户只给“仓库 + 场景”，代理必须按仓库规则自行启动，而不是再次索要内部文档路径
