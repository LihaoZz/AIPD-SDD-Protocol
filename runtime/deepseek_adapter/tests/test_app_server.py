...
ENTRYPOINT = ROOT / "bin" / "deepseek_app_server.py"


class DeepSeekAppServerTest(unittest.TestCase):
    def test_successful_turn_emits_completion(self) -> None:
        launcher = self._write_launcher(
            """
            #!/usr/bin/env python3
            import json
            print(json.dumps({"result": "DeepSeek success"}))
            """
        )
    ...
    def test_failed_turn_emits_failed_event(self) -> None:
        launcher = self._write_launcher(
            ...
            #!/usr/bin/env python3
            import sys
            sys.stderr.write("deepseek permission denied\\n")
            sys.exit(4)
            """
        )
        messages = self._run_session(launcher, approval_policy="never")
        ...
        self.assertEqual(messages[3]["params"]["exit_code"], 4)
        self.assertIn("deepseek permission denied", messages[3]["params"]["stderr"])

    def test_missing_launcher_fails_closed(self) -> None:
        missing = Path(tempfile.gettempdir()) / "missing-claude-deepseek"
    ...
    def _run_session(self, launcher: Path, approval_policy: str) -> list[dict[str, object]]:
        env = dict(os.environ)
        env["DEEPSEEK_CLAUDE_LAUNCHER"] = str(launcher)
                    ...
                    json.dumps(
                        {
                            "method": "thread/start",
                            "id": 2,
                            "params": {"cwd": "/tmp/project", "approvalPolicy": approval_policy},
                        }
                    ),
                    json.dumps(
                        {
                            ...
                            "params": {
                                ...
                                "input": [{"type": "text", "text": "Debug the backend mission block."}],
                            },
                        }
                    ),
                ]
    ...
    def _write_launcher(self, body: str) -> Path:
        temp_dir = Path(tempfile.mkdtemp(prefix="deepseek-launcher-"))
        launcher = temp_dir / "claude-deepseek"
