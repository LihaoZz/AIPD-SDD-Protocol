...
DEFAULT_LAUNCHER = Path("~/.claude/providers/launchers/claude-deepseek").expanduser()
...
class DeepSeekAppServer:
    ...
    def _handle_message(self, payload: dict[str, Any]) -> None:
        ...
        if method == "thread/start":
            ...
            self._send(
                {
                    "id": payload.get("id"),
                    "result": {"thread": {"id": f"deepseek-thread-{self._thread_counter}"}},
                }
            )
            return
        ...
        if method == "turn/start":
            ...
            turn_id = f"deepseek-turn-{self._turn_counter}"
            ...
            self._send(
                {
                    ...
                    "params": {
                        "turn_id": turn_id,
                        "message": "DeepSeek adapter only supports approvalPolicy=never in v1.",
        ...
        self._send(
            {
                ...
                "params": {
                    "turn_id": turn_id,
                    "message": result.text or "DeepSeek launcher failed.",
    ...
    def _invoke_launcher(self, prompt: str) -> LauncherResult:
        launcher = Path(os.environ.get("DEEPSEEK_CLAUDE_LAUNCHER", DEFAULT_LAUNCHER)).expanduser()

        if not launcher.is_file():
            return LauncherResult(
                success=False,
                text=f"DeepSeek launcher not found: {launcher}",
                stdout="",
                stderr="",
                exit_code=127,
            )
